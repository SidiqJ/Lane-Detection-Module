...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		minLX = leftx_current - margin
		maxLX = leftx_current + margin

		minRX = rightx_current- margin
		maxRX = rightx_current+ margin

		# Draw the windows on the visualization image using cv2.rectangle()
		##TO DO
		cv2.rectangle(out_img,(minLX,minY),(maxLX,maxY),(0,255,0),2)
		cv2.rectangle(out_img,(minRX,minY),(maxRX,maxY),(0,255,0),2)

		# Identify the nonzero pixels in x and y within the window
		##TO DO
		leftNZ = ((nonzeroy >= minY)&(nonzeroy<maxY)&(nonzerox >= minLX)&(nonzerox<maxLX)).nonzero()[0]
		rightNZ = ((nonzeroy >= minY)&(nonzeroy<maxY)&(nonzerox >= minRX)&(nonzerox<maxRX)).nonzero()[0]
		# Append these indices to the lists
		##TO DO
		left_lane_inds.append(leftNZ)
		right_lane_inds.append(rightNZ)
		# If you found > minpix pixels, recenter next window on their mean position
		##TO DO
		if len(leftNZ)> minpix:
			leftx_current = np.int(np.mean(nonzerox[leftNZ]))
		if len(rightNZ)> minpix:
			rightx_current = np.int(np.mean(nonzerox[rightNZ]))
